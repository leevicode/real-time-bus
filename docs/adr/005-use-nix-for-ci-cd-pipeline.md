...
## Context
Ubuntu runners use npm, pip, and system packages using apt-get. This approach usually leads to recurring issues: inconsistent dependency versions between local development and CI, "works on my machine" failures, slow pipeline runs due to full dependency re-installs on every job, and difficulty reproducing production build environments exactly. We need a reliable, reproducible, and cache-efficient CI/CD solution. We evaluated Docker-based approaches (using pre-baked images) and Nix. Docker offers better ecosystem maturity, but Nix provides stronger reproducibility guarantees at a finer granularity, potentially faster incremental builds, and atomic rollbacks for the build environment.
